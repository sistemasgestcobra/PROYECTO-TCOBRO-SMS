...
<strong class="text-primary">Historial Correos y SMS:</strong>
    <table id="table_clients_contact"
               data-side-pagination="server"
               data-toggle="table_clients_contact"
               data-height="<?php echo $data_height ?>"
               data-url="<?= base_url('cobranzas/credit/get_credit_hist_contact/'.$client_id) ?>"
               data-sort-name="id"
            ...
            ?>  
                    ...
                    data-detail-formatter="editTemplateContactNotif" 
                    data-pagination="true"               
                    ...
                    data-select-item-name="c_id_contactados[]"
                    data-id-field="id"
        ...
        >
            ...
            <tr>
                ...
                <th data-field="status" data-formatter="inputFormatStatus"  data-filter-control="input">Estado</th>
                <th data-field="contact" data-filter-control="input">Contacto</th>
                <th data-field="curr_date" data-filter-control="input">Fecha</th>                        
                <th data-field="curr_time"  data-filter-control="input">Hora</th>
            </tr>
            </thead>
        </table>    
</div>
...
<script>
    ...
    $('[data-toggle="table_clients_contact"]').bootstrapTable(); 
    var $table_clients_contact = $('#table_clients_contact');
    function setTotalSelecteds(){
        var data_clients_contact = $table_clients_contact.bootstrapTable('getData');            
//        console.log(data.toString());
    ...
    $table_clients_contact.on('load-success.bs.table', function (es) {
        setTotalSelecteds();  
        $('input[rel="txtTooltip"]').tooltip();        
    });
   <?php
    if( $show_export == 1 ){
      ?>
        function editTemplateContactNotif(index, row) {
        var html = [];
        var id = row['com_id'];
        var verMensajeEnviado = '<a id="call-php" class="" href="#" data-target="messagesout" php-function="<?= base_url() ?>modal/admin__notificationformat__open_view__'+id+'/0/0">Ver Mensaje</a>';
        html.push(verMensajeEnviado);
        return html.join('');
    }   
      <?php          
    }
    ?>
    function inputFormatStatus(index, row) {
