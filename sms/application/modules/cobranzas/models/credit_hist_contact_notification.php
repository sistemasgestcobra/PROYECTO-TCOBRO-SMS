...
class credit_hist_contact_notification extends \CI_Model {
    private function _get_credit_detail_filter( $filter = null,$client_id) {
			...
			$this -> db -> join('motivo_no_pago mot', 'mot.id = com.motivo_no_pago_id');
     ...
     */
        function credit_hist_notification_data($limit, $offset, $filter, $order_by,$client_id){
            ...
            $this->db->select(
                        'com.id,com.id com_id,com.type,com.status, mot.motivo_name, com.detalle_notificacion, com.contact, com.contact_id ,com.curr_date, com.curr_time, com.notificador,com.user_id, com.fecha_entrega, com.hora_entrega,c_r.id credit_detail_id,c_r.person_id, p.id person_id, c_d.nro_pagare,p.firstname person_name,', FALSE
                    );
            if($order_by){
