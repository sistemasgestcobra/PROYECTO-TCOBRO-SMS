    ...
    public function notifications_save() {
        ...
        if($credit_detail_check){
            foreach ($credit_detail_check as $credit_det) {
                ...
                $hola=$this->save_comunication('visita', $credit_det, '-1', '', "visita", '', $referencia_deudor, $notification_id_notification);
				
				$client = new SoapClient("http://192.168.187.152/FBSEventosBatch/ServicioGestCobra.asmx?WSDL");
				$uno=array("_numeroOperacion"=>$credit_data->nro_pagare,
						"_tipoNotificacion"=>$notification_id_notification);
				 $xml = $client->EnviarNotificacion($uno);
				 $xml2= $xml->EnviarNotificacionResult;
				 echo $xml2;
				 echo $kk= $credit_data->nro_pagare;
				 
				  //print_r($xml2);
				$credit_data->total_cuotas_vencidas=$xml2;
				$credit_data->save();
...
print_r($type."--". $referencia."--". $status."--". $contact."--". $notification_format_id);
        $comunication_type = (new gestcobra\comunication_type_model())
                ->where('comunication_code', $type)
                ->find_one();
