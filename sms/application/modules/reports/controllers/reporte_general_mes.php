...
class Reporte_general_mes extends CI_Controller {
...
function __construct() {
	...
	public function index()
	{
       ...
       // $oficina = 390;
        if  ($agencia==-1&&$mes==-1) {
        if (!$mysqli->multi_query("select   CONCAT(oc.name,CONCAT(' - '),m.month_name), rm.cantidad,rm.capital, rm.cantidad_x_recuperar, rm.capital_x_recuperar, rm.cantidad_recuperada, rm.capital_recuperada, ROUND((rm.capital_recuperada *100)/rm.capital,2)
from reporte_general_mensual rm ,oficina_company oc, month m WHERE oc.id=rm.oficina_company_id and m.id=rm.mes_id")) 
           
            {
            echo "Falló la llamada: (" . $mysqli->errno . ") " . $mysqli->error;
        }
        }elseif ($agencia==-1&&$mes!=-1) {
            if (!$mysqli->multi_query("select oc.name, rm.cantidad,rm.capital, rm.cantidad_x_recuperar, rm.capital_x_recuperar,rm.cantidad_recuperada, rm.capital_recuperada, ROUND((rm.capital_recuperada *100)/rm.capital,2)
...
$objActSheet->getColumnDimension('H')->setAutoSize(true);
        ...
        $fecha=date('Y/M/d');
        $nombre='ReporteMensual-'."$fecha".'.xlsx';
