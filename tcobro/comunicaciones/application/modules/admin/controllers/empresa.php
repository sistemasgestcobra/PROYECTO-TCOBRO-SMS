...
//                }else{
//                    $this->_send_confirmation_code( $ml_company->emails, $ml_company->activation_code );
//                    echo 'Su codigo de activacion ha sido enviado a su e-mail: '.$ml_company->emails;
//                }
        }
    }
     ...
     */
    private function _activate_users($company_id, $company_status_id) {
        $access_allowed = 1;
        if ($company_status_id != 1) {
            $access_allowed = 0;
        }
        $data = array(
        ...
        $this->db->update('ml_persona_acceso', $data);
    }

    private function _send_confirmation_code($email, $activation_code) {
        $this->load->library("email");
//                $configGmail = array(
//                    'protocol' => 'smtp',
//                    'smtp_host' => 'mail.guayacansoft.com',
//                    'smtp_port' => 465,
//                    'smtp_user' => 'noreply@guayacansoft.com',
//                    'smtp_pass' => 'guayacan2015',
//                    'mailtype' => 'html',
//                    'charset' => 'utf-8',
//                    'newline' => "\r\n"
//                );
//                $this->email->initialize($configGmail);
        ...
        $this->email->from('guayacansoft@guayacansoft.com', 'Guayacansoft');
        $this->email->to($email);
        $this->email->subject('Activacion');

        $msg = "Thanks for signing up!
            Your account has been created, 
            you can login with your credentials after you have activated your account by pressing the url below.
            Please click this link to activate your account:<a href='.base_url('user/verify').'/'.$activation_code.'>Click Here</a>";
...
//            $this->load->library('email');          
//            
//            $this->email->from('estyom.1@gmail.com', 'Guayacan Soft');
//            $this->email->to($email);
//            $this->email->cc('guayacansoft@guayacansoft.com');
//
//            $this->email->subject('Codigo de Activacion');
//            $this->email->message('Felicidades!!, ahora tienes una cuenta en el sistema de administracion empresarial Marilyn, tu codigo de activacion es: '.$activation_code);
//            $this->email->send();
     ...
     * @param type $company_id
     */
    private function _exec_company_transaction($file_path, $company_id) {
        ...
        foreach ($sqls as $statement) {
            $statment = $statement . ";";
            $statment2 = str_replace('company_id', $company_id, $statment);
            $this->db->query($statment2);
        }
    }

//        public function _exec_company_transaction( $file_path, $company_id ) {
//            $sql = file_get_contents($file_path);
//
//            $sqls = explode(';', $sql);
//            array_pop($sqls);
...
//        }
    ...
    public function switch_company($company_id) {
        ...
        $ml_company = new gestcobra\company_model($company_id);
//            $this->session->set_userdata( 'version', $ml_company->systemversion_id );
