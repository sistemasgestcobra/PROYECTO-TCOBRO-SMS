...
class Notificationformat extends CI_Controller {
    ...
    public function notification_print($id = 0, $credit_id = 0) {
        ...
        if (($notification_id == 'com_masiva')AND ( ($notificacion->type != 'NOTIFICACION'))) {
            ...
            // $res['id'] = set_post_value($notif_id[0], $id);
...
//          $res['view_name'] = 'admin/ml_notification_print';
//          $this->load->view( 'common/templates/dashboard', $res );
//          $res['c_d_id'] = set_post_value('c_d_id');
//          sleep(5);
...
//            write_file('./uploads/newpdf.pdf', $data);    
        }
    }
    ...
    public function update_comunicacion_new() {
        ...
        $ocho=set_post_value('valor_promesa');
        ...
        ?>
            ...
            private function save_credit_hist() {
            $credti_hist = new gestcobra\credit_hist_notification_model();
            $id=set_post_value('id');
            $comunication= (new gestcobra\comunication_model())
                ->where_in('id',$id)
                ->find_one();
            ...
            $credit_id=$cliente_ref->credit_detail_id;
                    
            
            $credti_hist->credit_detail_id = $credit_id;
            
            $credti_hist->detail = set_post_value('detalle_notificacion');
            ...
            $credti_hist->contact_id =set_post_value('contact_id');
            
            $cred=(new gestcobra\credit_detail_model())
                ->where('id',$credit_id)
                ->find_one();
            $oficina=$cred->oficina_company_id;
            
            $credti_hist->oficina_company_id=$oficina;
...
//$credti_hist->compromiso_pago_date = 'NULL';
            ...
            ?>
                    <script>
                        $("#table_credit_hist_notification").bootstrapTable('refresh');
                        $("#table_credits_hist_notification").bootstrapTable('refresh');
...
//        fclose($fp);
                    ...
                    foreach ($contact_model as $value) {
                        ...
                        foreach ($comunicacion_opt_client as $value_sms) {
                            ...
                            if (($value_sms == 'com_sms' AND $tipo_contacto_ofic == 2) OR ( $value_sms == 'com_call' AND ( $tipo_contacto_ofic == 2 OR $tipo_contacto_ofic == 1))) {
                                $from = $i;
                                $obj_commws->set_url_ws('http://localhost/comunicaciones/ws_sms/examplews/masivo');
    ...
    public function save(){
        ...
        if ($lote == 1) {
            ...
            $notification_format->oficial_credito_id = $this->user->id;
    ...
    public function save_update(){
        ...
        $notification = (new gestcobra\notification_format_model())
                ->where('description', $description[$cont])
                ->find_one();
        $var = $notification->description;

        //si es diferente de null
        if (!empty($var)) {
            $this->db->where_not_in('id', $ids);
            $this->db->where('oficial_credito_id', $this->user->id);
            $this->db->delete('notification_format', array('oficial_credito_id' => $this->user->id));
        } else {
            $this->db->where_not_in('id', $ids);
            $this->db->where('oficial_credito_id', $this->user->id);
            $this->db->delete('notification_format', array('oficial_credito_id' => $this->user->id));
            ...
            if (!empty($description[$cont]) AND $description[$cont] != 'undefined' AND $description[$cont] != 'null') {
                ...
                $notification->oficial_credito_id = $this->user->id;
