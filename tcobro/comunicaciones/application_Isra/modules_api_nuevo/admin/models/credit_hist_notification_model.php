...
class Credit_hist_notification_model extends \CI_Model {
    ...
    private function _get_hist_filter( $filter = null, $client_id, $compromiso_pago_date ) {
            ...
            if( $compromiso_pago_date != 0 ){
                $this -> db -> where ('c_h_n.compromiso_pago_date', $compromiso_pago_date);
            ...
            if($filter){
                foreach($filter as $key => $value){
                    if($key == 'client_name'){
                        ...
                        $this -> db -> like ('UPPER(c_h_n.detail)', strtoupper($value));
                    }elseif($key == 'hist_date'){
                        $this -> db -> like ('UPPER(c_h_n.hist_date)', strtoupper($value));
                    }elseif($key == 'hist_time'){
                        $this -> db -> like ('UPPER(c_h_n.hist_time)', strtoupper($value));
                    }elseif($key == 'compromiso_pago_date'){
                        $this -> db -> like ('UPPER(c_h_n.compromiso_pago_date)', strtoupper($value));
            ...
            $this -> db -> from('credit_hist_notification c_h_n');
            $this -> db -> join('credit_detail c_d', 'c_d.id = c_h_n.credit_detail_id');
            ...
            $this -> db -> join('credit_status c_s', 'c_s.id = c_h_n.credit_status_id');
            $this -> db -> join('oficial_credito o_c','o_c.id=c_h_n.oficial_credito_id');
    }
     ...
     */
        function get_hist_data( $limit, $offset, $filter = null, $order_by = null, $client_id, $compromiso_pago_date ) {
            ...
            $this -> db -> select(
                        'c_h_n.id, c_h_n.detail, c_h_n.hist_date, c_h_n.hist_time, c_h_n.compromiso_pago_date, c_h_n.credit_status_id, c_h_n.contact_id, c_h_n.valor_promesa ,c_d.nro_cuotas, c_d.nro_pagare, c_d.deuda_inicial, c_d.saldo_actual, c_d.adjudicacion_date, c_d.total_cuotas_vencidas, c_r.client_code, o_c.firstname, c_h_n.contact_id, CONCAT_WS("",cl_p.firstname," ",cl_p.lastname) client_name, c_s.status_name, c_s.color, c_s.background', FALSE
                    );
...
//            echo $this->db->last_query();
        ...
        $this->db->select(
                'c_h_n.id, c_h_n.detail, c_h_n.hist_date, c_h_n.hist_time, c_h_n.contact_id, c_h_n.compromiso_pago_date, cl_p.cedula_deudor,c_h_n.credit_status_id, c_d.nro_cuotas, c_d.nro_pagare, c_d.deuda_inicial, c_d.saldo_actual, c_d.adjudicacion_date, c_d.total_cuotas_vencidas, c_r.client_code, o_c.firstname,CONCAT_WS("",cl_p.firstname," ",cl_p.lastname) client_name, c_s.status_name, c_s.color, c_s.background', FALSE
        );
        if ($order_by) {
            foreach ($order_by as $order => $tipo) {
                $this->db->order_by($order, $tipo);
