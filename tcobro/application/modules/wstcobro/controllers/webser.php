...
return $array_1;
	...
	public function actualiza_get() {
		$web="http://192.168.187.152/FBSEventosBatch/ServicioGestCobra.asmx/ObtenerDatosModuloExtrajudicial";
		$lineas = file($web);
		
		//$lineas = array('41|21.87','40|6.87');
			...
			$credit_detail = (new gestcobra\credit_detail_model())
				...
				if ($credit_detail->id != 0){
					...
					if($pago < $pago_minimo){
						...
						$credit_detail->save();
						if($credit_detail->total_pagar >= $pago_minimo){
							$credit_detail->credit_status_id = 22;
							$credit_detail->total_pagar = 0.0;
							$credit_detail->save();
						}else{
