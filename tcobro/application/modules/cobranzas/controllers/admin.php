    ...
    function load_credits_data() {
        ...
        if (file_exists($logo_path . $upl_data['file_name'])) {
            ...
            for ($x = 2; $x <= $PHPExcel->getActiveSheet()->getHighestRow(); $x++) {
				...
				$comision = get_value_xls($PHPExcel, $this->abecedario["O"], $x);
             ...
             //  $oficial_credito_cedula = get_value_xls($PHPExcel, $this->abecedario["Y"], $x);
	          ...
	          $credit_detail->oficial_credito_id = 1384;
			  // $credit_detail->oficial_credito_id = $this->asignacion;
			   }
			   ...
			   // ACT. CREDITOS 
				...
				if ($total_cuotas_vencidas<$credit_detail->total_cuotas_vencidas){
					//crear abono
					$pago=$credit_detail->total_cuotas_vencidas-$total_cuotas_vencidas;
					
					//$comision=0;
					$comision=$this->comision( $pago,$dias_mora);
					  $this->guardar_abono($credit_detail->id, $pago,$dias_mora,$comision);
}
                }
				...
				$comision=$this->comision( $total_cuotas_vencidas,$dias_mora);
				$credit_detail->total_comision = $comision;
			$credit_detail->total_pagar = $credit_detail->total_comision+$credit_detail->total_cuotas_vencidas;
...
//				$credit_detail->oficial_credito_id = $oficial_credito_id;
				...
				$credit_detail->oficina_company_id =$oficina_id;
                ...
                foreach ($array_phone as $phone_contact) {
                      ...
                      if (strlen($phone_contact) > 6 AND strlen($phone_contact) <= 9) {
                        $this->_save_contact($res_deudor->person_id, $phone_contact, 1,2);
						  ...
						  if (strlen($phone_contact) >=10 AND strlen($phone_contact) <= 12) {
    ...
    private function _save_contact($person_id, $contact, $contact_type,$in) {
        if (!empty($contact)) {
		...
		$contact_data->contact_respuesta_id = $in;
        $contact_data->save();
    ...
    private function _get_oficina_id($oficina_name) {
	 ...
	 private function guardar_abono($id_credito, $pago, $dias_mora, $comision) { 
        $abono = (new gestcobra\abono_model());
        $abono->credit_detail_id = $id_credito;
        $abono->date_abono = date('Y-m-d', time());       
        $abono->amount = $pago;
		$abono->dias_mora = $dias_mora;
		$abono->valor_comision = $comision;
...
//if (!$mysqli->multi_query("UPDATE `credit_detail` SET `credit_status_id`='22' WHERE  `load_date`!='$fecha' and `credit_status_id`!='14'")){
if (!$mysqli->multi_query("UPDATE `credit_detail` SET `credit_status_id`='22' WHERE  `load_date`!='$fecha' and `credit_status_id`!='14'")){
            echo "Fallo la llamada: (" . $mysqli->errno . ") " . $mysqli->error;
    }
}
...
$aux=10000;
$oficail_as=0;
 $cred = $this->db->query("select oficial_credito_id, count(id) as num  from credit_detail where oficina_company_id=438 and oficial_credito_id!=1382 GROUP by oficial_credito_id  ORDER BY num asc ");
        $des = $cred->result();
        foreach ($des as $row) {
            $oficial = $row->oficial_credito_id;
            $can_creditos = $row->num;
			print_r($oficail."--".$can_creditos);
