...
array_push($numeros, $numero);
            ...
            for ($x = 2; $x <= $PHPExcel->getActiveSheet()->getHighestRow(); $x++) {
                ...
                if($client_referncia->id != 0 ){
					...
					$this->save_comision($credit_id->id);
                }
    ...
    function save_comision($id) {
         $credit_id = (new \gestcobra\credit_detail_model())
                        ->where('id', $id)
                        ->find_one();
                
        $credit_id->total_comision= $credit_id->total_comision+1;  
		$credit_id->total_pagar=$credit_id->total_comision+$credit_id->total_cuotas_vencidas; 
         $credit_id ->save();
    }
