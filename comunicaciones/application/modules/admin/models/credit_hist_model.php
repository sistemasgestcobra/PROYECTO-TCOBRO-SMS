...
class Credit_hist_model extends \CI_Model {
            ...
            if($filter){
                foreach($filter as $key => $value){
                    if($key == 'client_name'){
                        ...
                        $this -> db -> like ('UPPER(cl_p.personal_address)', strtoupper($value));
                    }elseif($key == 'detail'){
     ...
     */
        function get_hist_data( $limit, $offset, $filter = null, $order_by = null, $client_id, $compromiso_pago_date ) {
            ...
            $this -> db -> select(
                        'c_h.id, c_h.detail, c_h.hist_date, c_h.hist_time, c_h.compromiso_pago_date, cl_p.personal_address, c_h.credit_status_id, c_h.contact_id, c_h.valor_promesa ,c_d.nro_cuotas, c_d.nro_pagare, c_d.deuda_inicial, c_d.saldo_actual, c_d.adjudicacion_date, c_d.total_cuotas_vencidas, c_r.client_code, o_c.firstname, c_h.contact_id, CONCAT_WS("",cl_p.firstname," ",cl_p.lastname) client_name, c_s.status_name, c_s.color, c_s.background', FALSE
                    );
...
//            echo $this->db->last_query();
        ...
        $this->db->select(
                'c_h.id, c_h.detail, c_h.hist_date, c_h.hist_time, c_h.contact_id, c_h.compromiso_pago_date, cl_p.cedula_deudor,c_h.credit_status_id, c_d.nro_cuotas, c_d.nro_pagare, c_d.deuda_inicial, c_d.saldo_actual, c_d.adjudicacion_date, c_d.total_cuotas_vencidas, c_r.client_code, o_c.firstname,CONCAT_WS("",cl_p.firstname," ",cl_p.lastname) client_name, c_s.status_name, c_s.color, c_s.background', FALSE
        );
        if ($order_by) {
            foreach ($order_by as $order => $tipo) {
                $this->db->order_by($order, $tipo);
